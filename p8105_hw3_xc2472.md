...
represents a product from an order. The `aisle` variable represents the
type of the goods that are ordered, the `reordered` variable represents
if this product has been ordered by this user in the past, and
`order_day shows` the day of the week on which the order is placed. For
example, the first row in the dataset tells us yogurt was ordered on
